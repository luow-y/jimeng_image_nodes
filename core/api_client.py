...
class ApiClient:
    ...
    def _send_request(self, method, url, **kwargs):
        ...
        try:
            ...
            # 记录请求和响应信息（在raise_for_status之前，以便看到错误响应）
            logger.debug(f"[Jimeng] Request URL: {url}")
            logger.debug(f"[Jimeng] Request Method: {method}")
            logger.debug(f"[Jimeng] Response Status: {response.status_code}")
            if 'params' in kwargs:
            ...
            if not response.text or response.text.strip() == "":
                logger.error(f"[Jimeng] Empty response from API. Status: {response.status_code}")
                return None
            
            logger.debug(f"[Jimeng] Response Text: {response.text[:500]}...")  # 只记录前500字符
            ...
            try:
                ...
                logger.error(f"[Jimeng] Failed to parse JSON response: {json_err}")
                logger.error(f"[Jimeng] Response content: {response.text[:1000]}")
                return None
                
        except requests.exceptions.HTTPError as http_err:
            logger.error(f"[Jimeng] HTTP Error: {http_err}")
            logger.error(f"[Jimeng] Response: {response.text if response else 'No response'}")
            return None
        except requests.exceptions.RequestException as req_err:
            logger.error(f"[Jimeng] Request Exception: {req_err}")
            return None
        except Exception as e:
            logger.error(f"[Jimeng] Unexpected error in _send_request: {e}", exc_info=True)
            return None
    ...
    def generate_t2i(self, prompt: str, model: str, ratio: str, seed: int = -1, resolution: str = "2k"):
        ...
        try:
            ...
            # 记录submit_id，用于轮询（文生图更适合用submit_id）
            logger.info(f"[Jimeng] 文生图任务提交成功 - submit_id: {submit_id}, history_id: {history_id}")
            ...
            # 立即获取一次状态，检查排队信息
            # 优先使用submit_id查询（对文生图更可靠）
            first_check_result = self._get_generated_images_by_submit_id(submit_id)
            if not first_check_result:
                # 回退到history_id查询
                first_check_result = self._get_generated_images(history_id)
            queue_info = self._get_queue_info_from_response_by_submit_id(submit_id)
            ...
            for attempt in range(max_retries):
                ...
                # 优先使用submit_id查询（文生图推荐方式）
                image_urls = self._get_generated_images_by_submit_id(submit_id)
                if not image_urls:
                    # 回退到history_id查询
                ...
                if image_urls:
                    ...
                    return {"urls": image_urls, "history_record_id": history_id, "submit_id": submit_id}
    ...
    def _get_queue_info_from_response_by_submit_id(self, submit_id):
        """使用submit_id从API响应中获取排队信息（文生图推荐）"""
        try:
            url = f"{self.base_url}/mweb/v1/get_history_by_ids"
            ...
            data = {
                "submit_ids": [submit_id],  # 使用submit_ids而不是history_ids
                "image_info": {
                    "width": 2048,
                    "height": 2048,
                    "format": "webp",
            ...
            if result and result.get('ret') == '0':
                submit_data = result.get('data', {}).get(submit_id, {})
                queue_info = submit_data.get('queue_info', {})
                if queue_info:
                    return queue_info
                return None
                
        except Exception as e:
            logger.error(f"[Jimeng] Error getting queue info by submit_id: {e}")
            return None

    def _get_queue_info_from_response(self, history_id):
        """从API响应中获取排队信息（使用history_id）"""
        try:
            url = f"{self.base_url}/mweb/v1/get_history_by_ids"
